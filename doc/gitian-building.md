...
*Setup instructions for a gitian build of eSportscoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the eSportscoin
...
- [Building eSportscoin](#building-esportscoin)
...
Clone the git repositories for esportscoin and gitian and then checkout the esportscoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the esportscoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building eSportscoin
----------------------

To build eSportscoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the esportscoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/esportscoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/esportscoingame/eSportscoin
...
For example:
```bash
URL=https://github.com/konez2k/esportscoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit esportscoin=${COMMIT} --url esportscoin=${URL} ../esportscoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit esportscoin=${COMMIT} --url esportscoin=${URL} ../esportscoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit esportscoin=${COMMIT} --url esportscoin=${URL} ../esportscoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/esportscoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/esportscoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/esportscoin-build.assert
```
...
[esportscoin/gitian.sigs](https://github.com/esportscoingame/gitian.sigs/) repository, or if that's not possible to create a pull
request.
