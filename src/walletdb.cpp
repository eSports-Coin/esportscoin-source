...
void ThreadFlushWalletDB(const string& strFile)
{
    ...
    RenameThread("esportscoin-wallet");
...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Call eSportscoin with fAggressive=true to
    // get as much data as possible.
    // Rewrite esportscoind data to wallet.dat
    ...
    std::vector<CDBEnv::KeyValPair> esportscoindData;
    bool allOK = dbenv.eSportscoin(newFilename, true, esportscoindData);
    if (esportscoindData.empty()) {
        LogPrintf("eSportscoin(aggressive) found no records in %s.\n", newFilename);
        return false;
    }
    LogPrintf("eSportscoin(aggressive) found %u records\n", esportscoindData.size());
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, esportscoindData) {
        if (fOnlyKeys) {
